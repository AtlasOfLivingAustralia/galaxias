...
- Support for Darwin Core objects (via a new `S3` class `dwca`)
- Vignettes for formatting data for DwC (using `dplyr` syntax)
- Check data against Darwin Core standards (via `pointblank`)
- Source, adapt and save appropriate metadata using markdown (using `rmarkdown` or `Quarto`)
- Final export of a compressed DwC-A from the above components (`.zip` format)
...
You can then use the following functions to complete your archive (NONE COMPLETE):

- `check()` runs checks in the console (or optionally via API? or should that be `validate()`?)
