...
- A file that maps your column headings to those of the Darwin Core standard

`galaxias` helps you generate, store, modify, and validate these files. To get 
started, simply start a pipe using the `dwca()` function:
...
## Mapping to DwC
For many people, the challenging part of building a Darwin Core Archive will
be mapping your column names to the terms required by the standard. 

You can view terms using the `{tawnydragon}` package.

To map them to your columns, you have two choices:
- use a standard data manipulation pathway, using `base::colnames()` or 
  `dplyr::rename()` or `dplyr::mutate()`.
- use our inbuilt shiny app by calling `map_terms()`
