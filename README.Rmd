...
Standardise data according to Darwin Core Standard using `set_` functions.
...
Once standardised, specify that we wish to use `df_dwc` in a Darwin Core Archive with `use_data()`. This saves `df_dwc` with a valid csv file name and location.
...
After editing, specify that we wish to use "metadata.Rmd" in a Darwin Core Archive with `use_metadata()`. THis converts our 
