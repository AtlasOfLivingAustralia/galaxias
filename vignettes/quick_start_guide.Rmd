...
<!-- If the intention is that most users never interact directly with corella, I'd leave out any mention of it. It is, after all, a quick start guide. - SB-->
...
<!-- Is there a reason why we use `sequentional_id()` in the example below? I ask because the function documentation recommends `composite_id()` in the first instance - SB-->
...
<!-- I don't think this is an essential change, but I'm putting it down for consideration. The ALA uses kingdom and family for namematching in cases of ambiguity (I think), so it may be worth including family in addition to/instead of phylum in the example above - SB-->
...
<!-- `occurrences` doesn't exist; is it supposed to be `my_data_dwc`? - SB-->
...
format (`eml.xml`), and a schema document (`meta.xml`). The files can be edited at any time to update your final Darwin Core Archive.
