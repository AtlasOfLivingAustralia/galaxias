...
First, we can standardise our data to conform to the Darwin Core Standard. We'll do this by using `set_` functions automatically loaded with galaxias(imported from the [corella package](https://corella.ala.org.au/)) <!-- I can't decide if it's worth mentioning corella at all at this point-->. 

`set_` functions work a lot like `dplyr::mutate()`; we can use them to modify and create columns. 
The suffix of each `set_` function gives an indication of the type of data they accept (e.g., `set_coordinates()`, `set_scientific_name`). Their arguments are valid Darwin Core terms to use as column names. 
...
To use our standardised data in a Darwin Core Archive, we can select columns that use valid Darwin Core terms as column names. Invalid columns won't be accepted when we try to build our Darwin Core Archive. Our data is an Occurrence-based data set (each row contains information at the observation level, as opposed to Site/Survey level), so we'll select columns that match names in `occurrence_terms()`.
...
my_data_dwc <- occurrences |>
  select(any_of(occurrence_terms()))

my_data_dwc
```

Now we can specify that we with to use `my_data_dwc` in our Darwin Core Archive with `use_data()`, which saves this data in the 
...
Once our metadata statement is complete, we can spefify that we wish use it in 
...
document is a required file in a Darwin Core Archive. If it is missing, 
it will build one. We can also build a schema document ourselves using `use_schema()`.

At the end of this process, you should have a Darwin Core Archive zip file. 
You should also have `data-publish` folder containing 
standardised data files (e.g., `occurrences.csv`), a metadata statement in EML 
format (`eml.xml`) and a schema document (`meta.xml`). The files can be edited 
or updated to edit your final Darwin Core Archive.

## Share your archive

<!-- 
This is incomplete. Do we want a separate vignette on this?  
Will there be a place on GitHub to share data?
-->

The final step is to share your completed Darwin Core Archive with a 
data infrastructure like the Atlas of Living Australia. To share with the 
ALA, send your file and a brief description to [support@ala.org.au](mailto:support@ala.org.au).
