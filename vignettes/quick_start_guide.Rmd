...
NOTE TO SELF - this list is for own use - not a good way to arrange in QSG
The most commonly used functions are:

Survey Events
 - `use_eventID()` unique identifier for the observation event [eventID, parentEventID]
 - `use_occurrenceID()` unique identifier for each row
 - `use_eventDate()` to specify a date in the correct format
...
Taxon
 - `use_scientificName()` record the highest level of taxonomic specificity in the dataset
 - `use_taxonomy()` to specify higher taxonomic columns

Details of individual observations
 - `use_individuals()` attributes of individuals measured [lifeStage sex vitality reproductiveCondition]
 - `use_abundance()` to state how many animals were seen during the observation
 
Observer
 - `use_observer()` to specify who made the observation
 - `use_basisOfRecord()` to specify the observation method
