...
For this example, we'll use a dataset containing observations of frogs [in x area over x time]. Data were collected using 5-minute audio surveys, where each row documents whether each frog species was detected over that 5-minute recording, recorded as present (`1`) or absent (`0`). Our dataset consists of 3 spreadsheets: `species list`, `observations` and `sites`.

##### Species list

The `species list` spreadsheet lists the eight frog species recorded in this dataset. The `abbreviation` column contains the abbreviated column name used in the observations spreadsheet.
...
The `observations` spreadsheet contains columns that describe the sample's location (e.g. `site_code`, `water_type`, `veg_canopy`) and the occurrence of each frog species (e.g. `cpar`, `csig`, `limdum`). 
...
The `sites` spreadsheet contains columns that describe each site location (e.g. `depth`, `water_type`, `latitude`, `longitude`) and overall presence/absence of each frog species in a site (e.g. `cpar`, `csig`, `limdum`). 
