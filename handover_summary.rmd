...
- I use VScode with auto formatting to wrap lines at 80 characters
  - Purely stylistic, it's not a big deal, but just mentioning as an FYI in case
  you want to keep to that in the future.
  - Not sure how to do it in Rstudio, probably there is a package
  - Also as an FYI as to why some text outputs are wrapped in `paste0()` - it
  lets you break printed text into multiple lines, for readability in the code,
  without having the line breaks appear in the output.
- Top-to-Bottom Approach (Caller First): If you have a main function that calls
several helper functions, the main function would go at the top, followed by the
helpers.
